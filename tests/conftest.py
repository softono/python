...
FastAPI app via uvicorn as a subprocess on a test port (4399, distinct from
the dev port 4300) against the live shared Postgres DB, then yields an
...
async def _insert_verified_user_async(password: str, email: str, role: str = "USER") -> str:
    ...
    try:
        user_id = await conn.fetchval(
            ...
            VALUES ($1, true, 'Py', 'Tester', '9000000099', 'active', $2)
            RETURNING id
            """,
            email,
            role,
...
def insert_verified_user(password: str, email: str, role: str = "USER") -> str:
    """Directly inserts a fully-verified, active user + credential account
    (bypassing register/OTP) so authenticated-flow tests don't depend on
    reading real email OTPs. Returns the user id."""
    import asyncio
...
def make_verified_user():
    """Factory fixture: make_verified_user(password, email, role="USER") -> user_id."""
    return insert_verified_user
